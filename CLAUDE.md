...
6. **Surfaces** — `cmd/harness/` (CLI subcommand dispatch in `main.go`), `server/` (HTTP+SSE: `POST /v1/chat`, `GET /v1/sessions`, `/healthz`; `Server.Public` serves the default identity's `workspace/pub/` at the root, unauthenticated — the publish skill's target), `channel/telegram/`, `schedule/` (proactive cron-like runs; a task's `Deliver` target like `telegram:<chatID>` pushes its output through the channel registry — `channel/` defines `Deliverer` + `Registry`, `app.Deliverers` wires the built-ins (telegram, webhook, push/apns, app)), `task/` (background one-shot queue: `harness task add|list|show|drain`, the `background_task` tool lets the agent queue work for itself and `task_status` lets it see its own queue — registered per-profile in `app.Build` with the surface's `Spec.TaskDeliver` as the reply target; the daemon's worker in `cmd/harness/task.go` drains it and re-queues jobs interrupted by a restart). `harness pulse` (`cmd/harness/pulse.go`) installs a per-identity scheduled check-in (stable id `pulse-<profile>`, idempotent) driven by the `pulse` skill; an empty reply delivers nothing — silence is by design.
...
- **Everything pluggable is a dropped file, per-identity scopable.** Identities = `profiles/<name>.md`; skills = `skills/<name>/SKILL.md`; specialists = `agents/<name>.md`; MCP tools = `mcp.json`; exec plugins = executables in `plugins/` (`connector/plugin`: `spec`/`run <tool>`/`deliver <kind> <dest>` over stdio — tools namespaced `<plugin>__<tool>`, manifest `writes` feeds the permission gate, `delivers` kinds extend `-deliver` targets via the registry fallback in `app.Deliverers`; example in `plugins.example/`). Project-local dirs are scanned first, then the user-config dir.
